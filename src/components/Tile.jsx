...
const Tile = ({ tile }) => {
  ...
  return (
    ...
    >
      {/* Edit button */}
      <button
        onClick={(e) => { e.stopPropagation(); setEditorOpen(true, tile._id || tile.id); }}
      ...
      <button
        onClick={(e) => {
          e.stopPropagation();
          if (window.confirm('Delete this tile?')) deleteTile(tile._id || tile.id);
        }}
      ...
      {/* TEXT TILE */}
      {tile.type === 'text' && (
        <div className="p-4 text-white break-words whitespace-pre-wrap">{tile.content}</div>
      )}
